import { projectDocument } from "../capabilities.js";
import {
  UI_PROTOCOL_VERSION,
  type UiCapabilities,
...
import { assertValidDocument, validatePatchBatch } from "../validation.js";
    ...
    try {
      switch (message.type) {
        ...
        case "error": {
          ...
          this.#publish();
          return { applied: true, ...(message.documentId !== undefined ? { documentId: message.documentId } : {}) };
        }
        case "action":
        case "subscription":
        case "unsubscribe":
