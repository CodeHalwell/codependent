...
import { UI_CONFORMANCE_STORY } from "../../src/tooling/index.js";

import { createHostReactCapabilities, createWebviewCapabilities, WEB_PRIMITIVES } from "../../src/host-react/capabilities.js";
import { RemoteUiRenderer, type RemoteUiRecoveryRequest } from "../../src/host-react/renderer.js";
import { WEB_CONTRIBUTION_POINTS, WEB_SLOT_DEFINITIONS } from "../../src/host-react/slot-registry.js";
import { RemoteUiStore } from "../../src/host-react/store.js";
import { applyThemeTokens, applyWireTheme } from "../../src/host-react/theme.js";
...
describe("Shared Host React RemoteUiRenderer", () => {
  let container: HTMLDivElement;
  let root: Root;
  let store: RemoteUiStore;
  let events: UiEvent[];
  let recoveries: RemoteUiRecoveryRequest[];
  const capabilities = createHostReactCapabilities({ width: 1024, height: 768 });
  ...
  it("mounts every advertised host contribution point through its semantic adapter", () => {
    const webviewCaps = createWebviewCapabilities({ width: 1024, height: 768 });
    expect(webviewCaps.client).toBe("vscode");
  ...
  it("draws a layered SVG DAG with per-node selection and a list fallback", () => {
    ...
    const nodes = Array.from(container.querySelectorAll<SVGGElement>("g.ui-graph-node"));
  ...
  it("lays a horizontal graph out in columns and lists an unrankable one", () => {
    ...
    render();
    const columns = Array.from(container.querySelectorAll<SVGGElement>("g.ui-graph-node"))
  ...
  it("capability_denial_and_unknown_nodes_degrade_safely", () => {
    // 1. Denied capability: node requiring audioCapture or unsupported primitive with fallback
    const deniedNodeWithFallback: UiElementNode = {
      kind: "element",
      id: "denied-audio",
      type: "Audio",
      requires: [{ feature: "audioCapture" }],
      props: { src: "audio.mp3", alt: "Audio note" },
      fallback: {
        kind: "element",
        id: "denied-fallback",
        type: "Text",
        props: { value: "Fallback: audio capture is not permitted on this host" },
        children: [],
      },
      children: [],
    };

    // 2. Denied capability without fallback -> renders unsupported / degraded message safely
    const deniedNodeWithoutFallback: UiElementNode = {
      kind: "element",
      id: "denied-diff",
      type: "Diff",
      requires: [{ feature: "editorMutations" }],
      props: { patch: "+added line" },
      children: [],
    };

    // 3. Unknown node kind (not text and not element)
    const unknownKindNode = {
      kind: "future-streaming-fragment" as const,
      id: "future-node",
      label: "Streaming fragment v2",
      text: "Fallback streaming fragment text",
    } as unknown as UiNode;

    // 4. Unknown element type with fallback
    const unknownTypeWithFallback: UiElementNode = {
      kind: "element",
      id: "custom-component",
      type: "AcmeCustomWidget3D",
      props: { title: "Custom 3D Widget" },
      fallback: {
        kind: "element",
        id: "custom-fallback",
        type: "Text",
        props: { value: "3D preview not supported in this view" },
        children: [],
      },
      children: [],
    };

    // 5. Unknown element type without fallback
    const unknownTypeWithoutFallback: UiElementNode = {
      kind: "element",
      id: "unknown-type",
      type: "SuperpowersHologram",
      props: { label: "Hologram Display" },
      children: [
        { kind: "text", id: "holo-child", text: "Child content inside hologram" },
      ],
    };

    // 6. Interactive button to test invalid event payloads
    const buttonWithHandler: UiElementNode = {
      kind: "element",
      id: "action-button",
      type: "Button",
      props: { label: "Trigger Safe Action", action: "test.action" },
      children: [],
    };

    const rootDoc: UiDocument = documentWith({
      kind: "element",
      id: "degrade-root",
      type: "Stack",
      props: {},
      children: [
        deniedNodeWithFallback,
        deniedNodeWithoutFallback,
        unknownKindNode,
        unknownTypeWithFallback,
        unknownTypeWithoutFallback,
        buttonWithHandler,
      ],
    });

    // Mount document with a custom unregistered slot point:
    // This verifies unregistered slots mount safely without throwing!
    const customSlotPlacement = { point: "unregistered-custom-sidebar-slot" as any, extensionId: "degrade.test" };
    expect(() => {
      store.apply({ type: "snapshot", document: rootDoc }, customSlotPlacement);
    }).not.toThrow();

    // Rendering should succeed without throwing
    expect(() => render()).not.toThrow();

    // Verify fallback from denied capability rendered
    expect(container.textContent).toContain("Fallback: audio capture is not permitted on this host");

    // Verify unknown element with fallback rendered its fallback
    expect(container.textContent).toContain("3D preview not supported in this view");

    // Verify unknown element without fallback degraded to text label safely
    expect(container.textContent).toContain("Hologram Display");

    // Verify unknown node kind rendered safely
    expect(container.textContent).toContain("Fallback streaming fragment text");

    // Verify unregistered slot mounted safely in DOM
    const unregisteredSlot = container.querySelector('[data-ui-slot-adapter="unregistered-custom-sidebar-slot"]');
    expect(unregisteredSlot).not.toBeNull();
    expect(unregisteredSlot?.closest('[data-ui-host-region="primary"]')).not.toBeNull();

    const actionButton = container.querySelector<HTMLButtonElement>('[data-ui-node-id="action-button"] button');
    expect(actionButton).not.toBeNull();

    // Dispatching directly with circular payload or malformed payload must not throw
    expect(() => {
      act(() => {
        actionButton?.click();
      });
    ...
    // Also verify that when an unsupported element is rendered with wildcard primitives,
    // RemoteElement renders .ui-unsupported with unsupported note and children without throwing
    const wildcardStore = new RemoteUiStore({ ...capabilities, primitives: "*" });
    wildcardStore.apply({
      type: "snapshot",
    ...
    act(() => {
      root.render(<RemoteUiRenderer store={wildcardStore} capabilities={{ ...capabilities, primitives: "*" }} dispatch={(e) => events.push(e)} />);
    });
    expect(container.querySelector(".ui-unsupported")).not.toBeNull();
    expect(container.textContent).toContain("SuperpowersHologram is not supported");
    expect(container.textContent).toContain("Child content inside hologram");
  });
});
