...
**Status:** plan of record · **Supersedes:** [`16-master-implementation-plan.md`](16-master-implementation-plan.md)
...
| ID | Item | Location | Effort |
|---|---|---|---|
| **R-SEC-1** (B1) | **Scope escape via symlink after a `..` pop** — `canonicalize_lenient` canonicalizes only the deepest existing ancestor then appends the remainder lexically, so real components (including symlinks) after a `..` stay unresolved: `<worktree>/nope/../link/etc/passwd` classifies `Allowed`. File tools are saved by `secure_fs`'s `openat(NOFOLLOW)`; **`cwd` is not** — three sites classify then spawn with the **raw** path. Fix: resolve every remainder component, and switch the `cwd` sites from `classify` to `resolve`. | `daemon/src/policy/scope.rs:213-236`; `shell.rs:135-161`, `git.rs:62`, `unified_exec.rs:33`, `search.rs:143` | M |
...
| **R-COR-7** (C7) | **Paused agent runs are *failed* by restart recovery**, the opposite of the workflow layer which preserves them for explicit resume. Lost user work; pick one semantic. | `daemon/src/recovery.rs:78-89` vs `workflows.rs:249-262` | S |
...
Spec [13](13-remote-ui-authoring-sdk.md) as written. Land the protocol schema export first so
...
Spec [14](14-tauri-react-companion-client.md), milestones 1–6. Milestone 5 (shared React
