...
impl Painter<'_> {
    ...
    fn split_container(&mut self, node: &UiNode, area: Rect, depth: u16) {
        ...
        self.draw_border(node, area, border, None);
        let inner = layout::content_box(area, node.props.layout.as_ref(), border);
        let gap = layout::gap(node.props.layout.as_ref(), Axis::Horizontal);
        let usable = inner
            .width
        ...
        let count = u16::try_from(node.children.len())
            .unwrap_or(u16::MAX)
            .max(1);
        let widths = vec![usable / count; node.children.len()];
        let rects = layout::horizontal(inner, &node.children, &widths, gap);
        for (child, child_area) in node.children.iter().zip(rects) {
            self.node(child, child_area, depth + 1);
        }
    }
    ...
    fn graph(&mut self, node: &UiNode, area: Rect) {
        ...
        for (index, item) in data_items(node).iter().enumerate() {
            ...
            let label = item_label(item, index);
            let edges = object
                .and_then(|object| object.get("edges").or_else(|| object.get("targets")))
                .and_then(Value::as_array)
                .map(|values| values.iter().map(value_text).collect::<Vec<_>>().join(", "))
                .unwrap_or_default();
            if edges.is_empty() {
                lines.push(format!(
                    "{} {label}",
                ...
                lines.push(format!("{label}{arrow}{edges}"));
            }
        }
        if lines.is_empty() {
